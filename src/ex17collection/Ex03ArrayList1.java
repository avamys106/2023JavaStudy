...
-데이터의 중복 저장이 허용된다.
-데이터의 저장 순서를 보장한다.
-Array라는 이름처럼 "배열"의 특성을 가지고 있어 index를 통한 젖아 및
접근이 가능하다.
 */
...
//		list.add(5,"소방차"); //에러발생됨
		...
		중복저장 : List는 배열의 특성을 가지므로 데이터를 중복저장
			할 수 있다. 동일한 데이터라도 index로 구분할 수 있기 때문이다.
		 ...
		 */
		/*
		방법1 : 일반 for문 사용. 이때는 인덱스를 토앻 접근해야 하므로
			get() 메서드를 사용한다.
		 */
		System.out.println("\n[일반for문 사용]");
		...
		/*
		포함여부확인 : contains(인스턴스)
			특정 인스턴스가 저장되어 있는지 확인할때 사용한다.
			존재한다면 true를 반환한다.
		 */
		...
		방법1 : remove(인덱스)
			...
			삭제후에는 인덱스가 자동으로 부여된다.
			삭제 성공시 삭제된 인스턴스를 반환하므로 삭제 후
			즉시 확인할 수 있다.
		 */
		...
		삭제하기
		방법2 : indexOf(인스턴스)
			위치를 모르는 경우에는 indexOf()를 통해
			해당 인스턴스의 인덱스를 찾은 후 삭제한다.
		 */
		...
		/*
		삭제하기
		방법3 : 인스턴스의 참조값을 통해 삭제한다. 이 경우에는 삭제에

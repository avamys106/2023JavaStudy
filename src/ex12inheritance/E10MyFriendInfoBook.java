...
class FriendInfoHandler {
	...
	public void addFriend(int choice) {
		...
		//고등학생 or 대학생 모두 기본정보가 있으므로 먼저 입력받는다.
		Scanner scan = new Scanner(System.in);
		...
		//입력선택에 따라 고등학생 또는 대학생으로 분기하여 입력받는다.
		if(choice==1) {
			//고등학생을 선택한경우 별명을 추가로 입력받는다.
	...
	public void showSimpleData() {
		...
		정의하지 않았다면, 자식클래스에서는 오버라이딩을 할 수 없으므로
		개별적인 멤버메서드를 정의해야한다.
		정보가 저장되는 배열은 Friend 타입이므로 자식인스턴스를
		저장하면 자동으로 형변환(업캐스팅)이 된다. 이때 자식의 정보를
		출력하는 메서드를 바로 호출할 수 없으므로 아래와 같이 일일히
...
//		}
	...
	public void searchInfo() {
		...
		for(int i=0 ; i<numOfFriends ; i++) {
			...
			멤버변수에 접근한다. 검색을 위해 임력한 이름과 비교해서 
